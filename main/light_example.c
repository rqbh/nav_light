...
extern void opencv_task();
extern void easyPR_task();
...
static void root_write_task( void* arg )
{
    ...
    while ( mwifi_is_connected() && esp_mesh_get_layer() == MESH_ROOT ) {
        ...
        MDF_LOGD( "Root receive, addr: " MACSTR ", size: %d, data: %.*s", MAC2STR( src_addr ), size, size, data );
...
static void root_read_task( void* arg )
{
    ...
    while ( mwifi_is_connected() && esp_mesh_get_layer() == MESH_ROOT ) {
        ...
        MDF_LOGD( "Root send, addrs_num: %d, addrs_list: " MACSTR ", size: %d, data: %.*s", httpd_data->addrs_num, MAC2STR( httpd_data->addrs_list ), httpd_data->size, httpd_data->size,
                  httpd_data->data );

        mwifi_type.group = httpd_data->group;
        memcpy( &mwifi_type.custom, &httpd_data->type, sizeof( mlink_httpd_type_t ) );

        ret = mwifi_root_write( httpd_data->addrs_list, httpd_data->addrs_num, &mwifi_type, httpd_data->data, httpd_data->size, true );
...
void node_handle_task( void* arg )
{
    ...
    while ( true ) {
        ...
        MDF_ERROR_GOTO( ret != MDF_OK, FREE_MEM, "<%s> Receive a packet targeted to self over the mesh network", mdf_err_to_name( ret ) );
        ...
        MDF_ERROR_GOTO( header_info->format != MLINK_HTTPD_FORMAT_JSON, FREE_MEM, "The current version only supports the json protocol" );
        ...
        if ( header_info->resp ) {
            ...
            if ( handle_data.resp_fromat == MLINK_HTTPD_FORMAT_HEX ) {
                ...
                MDF_LOGI( "Node send, size: %d, data: %.*s", handle_data.resp_size, handle_data.resp_size, handle_data.resp_data );
            }
        }
...
static mdf_err_t event_loop_cb( mdf_event_loop_t event, void* ctx )
{
    ...
    case MDF_EVENT_MWIFI_PARENT_CONNECTED:
        ...
        if ( !esp_mesh_is_root() ) {
            mlink_ble_config_t config = {
                 ...
                 */
                .custom_data = { 0x02, 0x01, 0x06, 0x1A, 0xFF, 0x4C, 0x00, 0x02, 0x15, 0xFD, 0xA5, 0x06, 0x93, 0xA4, 0xE2,
                                 0x4F, 0xB1, 0xAF, 0xCF, 0xC6, 0xEB, 0x07, 0x64, 0x78, 0x25, 0x27, 0xB7, 0xF2, 0x06, 0xC5 },
                .custom_size = 30,
            };
    ...
    case MDF_EVENT_MWIFI_ROOT_GOT_IP: {
        ...
        if ( !g_root_write_task_handle ) {
            xTaskCreate( root_write_task, "root_write", 4 * 1024, NULL, CONFIG_MDF_TASK_DEFAULT_PRIOTY, &g_root_write_task_handle );
        }

        if ( !g_root_read_task_handle ) {
            xTaskCreate( root_read_task, "root_read", 4 * 1024, NULL, CONFIG_MDF_TASK_DEFAULT_PRIOTY, &g_root_read_task_handle );
        }
...
void app_main()
{
    ...
    if ( mdf_info_load( "ap_config", &ap_config, sizeof( mwifi_config_t ) ) != MDF_OK ) {
        ...
        MDF_LOGI( "mconfig, ssid: %s, password: %s, mesh_id: " MACSTR, ap_config.router_ssid, ap_config.router_password, MAC2STR( ap_config.mesh_id ) );
     ...
     * @brief Handling data between wifi mesh devices.
     */
    xTaskCreate( node_handle_task, "node_handle", 4 * 1024, NULL, CONFIG_MDF_TASK_DEFAULT_PRIOTY, NULL );

    /**
     * @brief Periodically print system information.
     */
    TimerHandle_t timer = xTimerCreate( "show_system_info", pdMS_TO_TICKS( 10000 ), true, NULL, show_system_info_timercb );
    xTimerStart( timer, 0 );
}
