...
void command_line_handler( int argc, const char* argv[] )
{
    ...
    options.add_subroutine( "ann", "ann operation" ).make_usage( "Usages:" );
    {
        ...
        */
        ...
        options( ",ann", easypr::kDefaultAnnPath, "the ann model file you want to save" );
        options( "t,test", "run test in --chars" );
    }

    options.add_subroutine( "locate", "locate plates in an image" ).make_usage( "Usage:" );
    {
        /* ------------------------------------------
    ...
    options.add_subroutine( "judge", "determine whether an image block is the license plate" ).make_usage( "Usage:" );
    {
        /* ------------------------------------------
    ...
    program_options::select( subname )
        ...
        .found( "ann",
                [&]() {
                    ...
                    easypr::AnnTrain ann( parser->get( "chars" )->c_str(), parser->get( "ann" )->c_str() );
        ...
        .found( "judge",
                [&]() {
                    ...
                    if ( parser->has( "file" ) ) {
                        ...
                        std::cout << true_or_false[ easypr::api::plate_judge( image.c_str(), svm.c_str() ) ] << std::endl;
                    }
                } )
        .found( "recognize",
                [&]() {
                    ...
                    if ( parser->has( "path" ) ) {
                        if ( parser->has( "batch" ) ) {
                            ...
                            if ( parser->has( "color" ) ) {
                                ...
                                std::cout << colors[ easypr::api::get_plate_color( image.c_str() ) ] << std::endl;
                            }
                            else {
                                // return strings
                                auto svm = parser->get( "svm" )->val();
                                ...
                                auto results = easypr::api::plate_recognize( image.c_str(), svm.c_str(), ann.c_str() );
                                for ( auto s : results ) {
                                    std::cout << s << std::endl;
                                }
                            }
...
extern "C" void easyPR_task( void* p )
{
#if 0
  std::shared_ptr<easypr::Kv> kv(new easypr::Kv);
  kv->load("resources/text/chinese_mapping");
